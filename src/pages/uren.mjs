...
<section>
  ...
  <p class="sub">De opgaven van ${num(w('uren','eigenaren'))} huisarts-eigenaren in
  ${num(w('uren','praktijken'))} praktijken. De gecapte som komt uit op ${num(w('uren','fte_gecapt'),2)} fte,
  precies het getal dat de NZa in Tabel 9 publiceert.</p>
  ${panel(dataTable(T.opgave_verdeling, [null, num, v=>pct(v,1)]))}
...
<section>
  ...
  ${panel(dataTable(T.maatschappen, [null, num, num, v=>pct(v,1)]))}
...
<section>
  ...
  telt niet meer mee. In de steekproef gaat het om ${num(w('uren','boven_cap'))} van de
  ${num(w('uren','eigenaren'))} eigenaren, gemiddeld ${num(w('uren','boven_cap_uren'),1)} uur per week boven de grens.</p>
  ${panel(compareBars({
    items:[
    ...
    ], fmt:v=>num(v,1), eenheid:' fte', caption:'Fte praktijkhoudend huisarts in de steekproef van 306 praktijken.'
  }))}
